...
const errors = reactive({ email: '', code: '', newPassword: '' })
...
function validate() {
  ...
  errors.newPassword =
    form.newPassword.length >= 6 && form.newPassword.length <= 32 ? '' : '新密码 6~32 位'
  return !errors.email && !errors.code && !errors.newPassword
}
...
<template>
  <AuthShell title="重置密码" subtitle="通过邮箱验证码找回你的账号">
    <form class="form" @submit.prevent="onSubmit">
      ...
      >
        <template #suffix>
          <button type="button" class="code-btn" :disabled="count > 0 || sending" @click="sendCode">
      ...
      <p class="foot">想起来了？<router-link to="/login">返回登录</router-link></p>
    </form>
  </AuthShell>
</template>
